...
import Script from 'next/script'; // Import eklendi
import WhatsAppButton from '../components/WhatsAppButton';

export default function Home() {
  // Burger menü ve Popup için state'ler
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [seciliHizmet, setSeciliHizmet] = useState<any>(null);

  // Hizmetler verisi (Fotoğraf ve detaylarıyla)
  const tumHizmetler = [
    { 
      t: "Su Kaçak Tespiti", 
  ...
  return (
    <div className="bg-white min-h-screen text-gray-900 font-sans selection:bg-[#3b82f6] selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="md:hidden p-2 text-gray-900 focus:outline-none" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">{isMenuOpen ? <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /> : <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />}</svg>
            </button>
          </div>
        </div>
        {isMenuOpen && (
      ...
      {/* (Geri kalan orijinal Hero, Hizmetler, Footer ve Modal kodun burada aynen duruyor) */}
      {/* ... (Senin orijinal kodun buraya devam ediyor, silmedim) ... */}
